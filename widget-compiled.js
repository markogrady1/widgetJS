...
var Widget = function () {
	...
	function apiRequest(user, urlData, callback) {
		...
		var url = "https://api.github.com/" + urlParam + "/" + user.replace("/", "");
		const target = urlData.targetParam.length === 0 ? "" : "/" + urlData.targetParam.replace("/", "") + "?sort=updated";
		url = url + target;
	...
	function element(param, arr) {
		var tmp = typeof arr[0] == "object" ? createRepoElem(param, "li", 3, arr, true) : createElem(param, "li", 3, arr, true);
	}

	function view(param, arr, selector) {
		var tmp = typeof arr[0] == "object" ? repo(arr) : overview(arr, selector);
	}
	...
	function addAvatar(element, data) {
		...
		elem.innerHTML = tags[0] + "a href=" + link + " target=__blank" + tags[1] + tags[0] + "img class=avatar src=" + avatar + tags[1] + tags[3] + "a" + tags[1];
		attr(elem, "class", "header");
		var tmpElement = typeof data[0] !== "object" ? attr(elem, "class", "header") : attr(elem, "class", "header-repo");
	}
	...
	function createRepoElem(baseEl, elem, index, data, event) {
		for (var i = 0; i < data.length; i++) {
			...
			const icon = data[i].fork ? tags[0] + "span class='octicon octicon-repo-forked icon'" + tags[1] + tags[3] + "span" + tags[1] : tags[0] + "span class='octicon octicon-repo icon'" + tags[1] + tags[3] + "span" + tags[1];
